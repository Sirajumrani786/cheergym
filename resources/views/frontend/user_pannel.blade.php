...
<main>
    <div class="profile-container">
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form method="POST" action="{{ route('fuser.update', Auth::user()->id) }}">
                            @csrf
                ...
                <div class="PageWrapper__StyledPageWrapper-sc-3p8f0h-0 lcpsHk">
                                    ...
                                    <div class="SchoolRating__SchoolRatingContainer-sb9dsm-0 inMLDw">
                                        <div class="SchoolRatingSuperHeader__SuperHeaderContainer-dkqn0d-0 ikECew">
                                            <div class="RatingSuperHeader__SuperHeaderBody-fm303l-1 dkybbj">
                                                ...
                                                <a
                                                    class="RatingSuperHeader__StyledEditLink-fm303l-7 hpzdKZ"href="{{ route('edit_review', $review->id) }}">
                                                ...
                                                <div class="SchoolRating__RatingHeader-sb9dsm-5 yjiuU">
                                                    <div
                                                        ...
                                                        @if ($review->created_at)
                                                            {{ $review->created_at->format('M jS, Y') }}
                                                        @endif
                                                    </div>
                                                </div>
                    ...
                    @else
                        <div class="no-rating">
                            ...
                            <p>{{ Auth::user()->name }} hasn't given any ratings yet.</p>
                        </div>
                    @endif
                </div>
            </div>
        ...
        <div id="saved-gyms" class="content-section" style="display: none; overflow-y: auto; max-height: 80vh;">
            @if (Auth::check() && Auth::user()->savedGyms !== null)
                <h3>{{ count(Auth::user()->savedGyms ?? []) }} Saved Gyms</h3>
            @else
                <h3>0 Saved Gyms</h3>
            @endif

            @if (Auth::check() && !empty(Auth::user()->savedGyms))
                @foreach (Auth::user()->savedGyms as $savedGym)
                    @php
                        $facilitiesTotal = 0;
                        $coachingTotal = 0;
                        $atmosphereTotal = 0;
                    ...
                    @foreach ($savedGym->gym->reviews as $review)
                        @php
                    ...
                    <div class="gym-container">
                        <div class="gym-rating">
                            <span>
                                {{ number_format(($averageFacilities + $averageCoaching + $averageAtmosphere + $averageOverall) / 4, 1) }}</span>
                        </div>
                        <div class="gym-info">
                            <h4><a href="{{ route('gym', $savedGym->gym->id) }}"
                                    class="text-decoration-none text-dark">
                                    {{ $savedGym->gym->name }} </a></h4>
                            <p>{{ count($savedGym->gym->reviews ?? []) }} ratings</p>
                        </div>
                    </div>
                @endforeach
            @else
                <div class="no-saved-gyms text-center p-5">
                    <h4>{{ Auth::user()->name }} doesn't have any saved gyms yet</h4>
                </div>
            @endif
        </div>
    </div>
